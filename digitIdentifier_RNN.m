...
%% Define variables
maxEpochs = 5; % Define max epochs, used to train network
learningRate = 0.1; % Define learning rate, used to train network

%% Import data
[images,correct]= digitTrain4DArrayData; %Import training data (28x28x1x5000 = 5000 28x28 grayscale images)

%% Look at training images with label
lookImages = input('Look at training images? [y or n]','s');

if lookImages == 'y'
    for i = 1:5000
        imshow(images(:,:,1,i)); 
        title(string(correct(i)));
        pause(1)
    end
end
...
if strcmpi(trainAgain,'y')==1  %if the user wants to retrain the network
    ...
    %Set training parameters, # epochs, learning rate
    options=trainingOptions('sgdm','Plots','training-progress','MaxEpochs',maxEpochs,'InitialLearnRate',learningRate,'Shuffle','every-epoch');
